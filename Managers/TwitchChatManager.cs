...
namespace TwitchLeagueQueueBot.Managers
{
    ...
    public static class TwitchChatManager
    {
        ...
        public static DataGridView QueuedUsers;
        ...
        {
            if (string.IsNullOrEmpty(message) || (sendToLoL && !message.Trim().StartsWith("!queue"))) return;
            if (!sendToLoL)
            {
            ...
            else
            {
                ...
                {
                    Form1.Instance.AddToQueue(fromUser, message.Replace("!queue", "").Trim());
                }
                else
                {
